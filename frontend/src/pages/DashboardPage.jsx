...
export function DashboardPage() {
  ...
  const handleShare = async (cv, e) => {
    e.stopPropagation()
    try {
      const { data } = await cvApi.toggleShare(cv.id)
      setCvs(prev => prev.map(c => c.id === cv.id ? { ...c, isPublic: data.isPublic, shareToken: data.shareToken } : c))
      if (data.isPublic) {
        const link = `${window.location.origin}/shared/${data.shareToken}`
        navigator.clipboard.writeText(link)
        alert(t.shareLink + ' ' + link + ' (Kopyalandı!)')
      } else {
        alert(t.unshareBtn + ' ✓')
      }
    } catch {
                        ...
                        <div className="db-cv-footer">
                          ...
                          <div className="db-cv-actions">
                            <button
                              ...
                              onClick={e => handleShare(cv, e)}
                              style={{ color: cv.isPublic ? '#10b981' : undefined }}
                            >
                              {cv.isPublic ? (isTR ? '🔗 Linki Kopyala' : '🔗 Copy Link') : (isTR ? '🔗 Paylaş' : '🔗 Share')}
                            </button>
                            <button
                              className="db-cv-btn-copy"
